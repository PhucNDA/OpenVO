...
for _dir in eval_dirs:
    ...
    for scene in scenes:
        if "KITTI" in scene:
            ...
            gt_dir = nusc_gt_dir
        elif "ARGO2_Stereo" in scene:
            gt_dir = argo2_stereo_gt_dir
        else:
            print('No Dir!')
            continue

        epochs = sorted(glob.glob(os.path.join(result_dir_openvo, scene, _dir, "*")))
        
        json_path = f"./evaluation_results/{_dir}/{scene}.json"
        results = {}
        for ep in tqdm(epochs):
            result_dir = "{}".format(ep)
            if not os.path.exists(result_dir):
                continue

            if ep.split('/')[-1] in results.keys():
                continue
            if "ARGO2_Stereo" in scene:
                seqq = None                                                         
            else:
                seqq = None 
            ep_results = eval_tool.eval(gt_dir, result_dir, alignment=None, seqs=seqq)
            # ep_results = eval_tool.eval(gt_dir, result_dir, alignment='plain')
            # ep_results = eval_tool.eval(gt_dir, result_dir, alignment='scale')
