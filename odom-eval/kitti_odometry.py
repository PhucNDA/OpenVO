...
class KittiEvalOdom():
    ...
    def eval(self, gt_dir, result_dir, 
        ...
        seq_dir = sorted(glob(os.path.join(result_dir, "*.txt"))) # used to be gt_dir
    ...
    def eval_ts(self, gt_dir, result_dir, 
                alignment=None,
                seqs=None, time_step=2):
        """Evaulate required/available sequences
        Args:
            gt_dir (str): ground truth poses txt files directory
            result_dir (str): pose predictions txt files directory
        ...
        for i in self.eval_seqs:
            ...
            # ---- NEW BLOCK: match higher-rate GT to lower-rate predictions ----
            gt_keys = sorted(poses_gt.keys())
            pred_keys = sorted(poses_result.keys())

            N_gt = len(gt_keys)
            N_pr = len(pred_keys)

            # Only try to resample if lengths differ
            if N_gt != N_pr:
                if N_gt <= 1 or N_pr <= 1:
                    raise ValueError(f"Too few poses: gt={N_gt}, pred={N_pr}")

                if time_step is not None and time_step > 1:
                    # ratio = time_step
                    # expected_N_gt = (N_pr - 1) * ratio + 1
                    # if expected_N_gt != N_gt:
                    #     raise ValueError(
                    #         f"Lengths not compatible with time_step={time_step}: "
                    #         f"gt={N_gt}, pred={N_pr}, expected gt={expected_N_gt}"
                    #     )
                    pass
                else:
                    # infer ratio from lengths: (N_gt-1) / (N_pr-1)
                    num = N_gt - 1
                    den = N_pr - 1
                    if num % den != 0:
                        raise ValueError(
                            f"GT/pred lengths not compatible: gt={N_gt}, pred={N_pr}"
                        )
                    ratio = num // den
                ratio = time_step
                new_poses_gt = {}
                for j, k_pred in enumerate(pred_keys):
                    idx_gt = j * ratio          # e.g. 0,2,4,... for ratio=2
                    if idx_gt >= N_gt:
                        break
                    k_gt = gt_keys[idx_gt]
                    new_poses_gt[k_pred] = poses_gt[k_gt]

                poses_gt = new_poses_gt
