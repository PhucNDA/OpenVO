...
class Attention(nn.Module):
    ...
    @torch.jit.ignore()
    def forward(self, x: torch.Tensor, y: torch.Tensor = None, z: torch.Tensor = None) -> torch.Tensor:
        B, N, C = x.shape
        if y is None:
            y = x
        if z is None:
        ...
        k = self.k(y).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        v = self.v(z).reshape(B, N, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        # q, k, v = qkv.unbind(0)
...
class Block1(nn.Module):
    ...
    def forward(self,  x) -> torch.Tensor:
        # import pdb; pdb.set_trace()
        x,y,z = x
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), self.norm1(y), self.norm1(z))))
...
class VisionTransformer_cross(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            block_fn: Type[nn.Module] = Block1,
            mlp_layer: Type[nn.Module] = Mlp,
            x_patchify: bool = False,
            y_patchify: bool = False,
            z_patchify: bool = False,
    ) -> None:
        """
        Args:
        ...
        self.x_patchify = x_patchify
        self.y_patchify = y_patchify
        self.z_patchify = z_patchify
    ...
    def forward_features(self,  x: torch.Tensor, y: torch.Tensor = None, z: torch.Tensor = None) -> torch.Tensor:
        if y is None:
            y = x
        if z is None:
            z = x
...
@torch.no_grad()
def _load_weights(model: VisionTransformer_cross, checkpoint_path: str, prefix: str = '') -> None:
...
def _create_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_cross:
